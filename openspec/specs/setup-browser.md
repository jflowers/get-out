# setup-browser Specification

The `setup-browser` command is a 5-step interactive wizard that launches Chrome with a dedicated profile, guides the user through Slack authentication, and validates credentials.
...
The `setup-browser` command MUST execute 5 active wizard steps in sequence:

1. **Chrome profile** — Check/create dedicated profile directory at `~/.get-out/chrome-data/`
